...
//$bot_token = '1002606161:AAH3YxTeKBq8jf5YaTaEiB7OxR-llnE_ccU';
...
if ($text == '/start') {
    $option = [[$telegram->buildKeyboardButton("Русский 🇷🇺"), $telegram->buildKeyboardButton("O'zbek tili 🇺🇿")],];
    $keyb = $telegram->buildKeyBoard($option, $onetime=false, $resize=true);
    $content = array('chat_id' => $chatID, 'reply_markup' => $keyb, 'text' => "Пожалуйста выберите язык.
 \nIltimos, tilni tanlang.");
    $telegram->sendMessage($content);

//    msendKeyBoard(["Русский 🇷🇺", "O'zbek tili 🇺🇿"], "Пожалуйста выберите язык.Iltimos, tilni tanlang.");
} elseif ($text == "Русский 🇷🇺") {
    $user->setLanguage('ru');
